...
echo "== [1] Delete vectordb collection via orchestrator API =="
RAW="$(curl -sS -m 60 -w $'\n%{http_code}' \
  -X POST "${BASE_URL}/api/v1/orchestrator/vectordb/collections/delete" \
...
if [[ "$HTTP_CODE" != "200" ]]; then
  echo "delete collection API failed with HTTP ${HTTP_CODE}" >&2
  exit 1
fi
...
if [[ "$STATUS" != "true" ]]; then
  echo "delete collection status is not true" >&2
  exit 1
fi

echo "delete collection API passed."
