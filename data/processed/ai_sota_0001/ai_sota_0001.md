...
Runjia Yang<sup>1</sup> and Beining Shi<sup>2</sup>
...
$$MOM\_nM = \sum_{\text{Past}n*21 \text{ days}} R_d - \sum_{\text{Past } 0.1*n*21 \text{ days}} R_d$$

where R<sup>d</sup> is the daily return.
...
From this table, we can tell that by using the MOM 7M factor, we can reach a maximum annual return rate of 21.19% and a maximum Sharpe ratio of 0.62. It is also interesting that we find the momentum factor with a short time period, for example, MOM 1M and MOM 2M, have a very small even negative return rate. However, it exactly conforms to the short term reversion effect that the typical momentum factor would exclude.
...
$$REV\_nD = -\sum_{\text{Past } n \text{ days}} R_d$$
...
$$\label{eq:profit_margin} \begin{aligned} \text{Profit Margin} &= \frac{\left(\text{Total Revenue - Total Expenses }\right)}{\text{Total Revenue}} \end{aligned}$$
