...
func (uc *trainingFileUseCase) embedTextAsyncByKafka(ctx context.Context, uuid string, texts []string) ([][]float32, error) {
	if len(texts) == 0 {
	...
	correlationID := fmt.Sprintf("embed-text-%s-%d", uuid, time.Now().UnixNano())
	...
	if err := uc.KafkaPublisher.Publish(ctx, ports.PublishMessageInput{
		...
		return nil, fmt.Errorf("publish embedding text request: %w", err)
	}

	res, err := uc.pollEmbeddingResult(ctx, topicRes, "training-file-embed-text-"+uuid, correlationID)
	if err != nil {
		return nil, err
	}
	if strings.EqualFold(strings.TrimSpace(res.Status), "failed") {
		return nil, fmt.Errorf("embedding text failed: %s", res.Message)
	}
	if len(res.Embeddings) != len(texts) {
		return nil, fmt.Errorf("embedding result size mismatch: expected=%d got=%d", len(texts), len(res.Embeddings))
	}
