...
import (
	...
	use_cases "rag_imagetotext_texttoimage/internal/application/use_cases"
)
...
func main() {
	...
	collectionstore := qdrant.NewCollectionStore(
		client.Raw(),
		appLogger,
	)

	if exis, _ := collectionstore.CollectionExists(ctx, collectionName); exis == true {
		collectionstore.DeleteCollection(ctx, collectionName)
	}
	...
	collectionstore.EnsureCollection(ctx, collectionCongif)
	...
	searchEngineer := use_cases.NewSearchWithVectorDB(appLogger, qdrantpointStore)

	query := buildFakeQuery()

	results, err := searchEngineer.Search(ctx, query)
	if err != nil {
		panic(err)
	}
	for _, result := range results {
		if result.Point == nil {
			fmt.Printf("score=%.4f point=nil\n", result.Score)
			continue
		}
		fmt.Printf("id=%s score=%.4f modality=%s text=%q\n",
			result.Point.ID,
			result.Score,
			result.Point.Payload.Modality,
...
func buildFakeQuery() ports.SearchQuery {
	scoreThreshold := float32(0.6)
	req_1 := ports.SearchQuery{
		QueryText: "what is transformer",
		CollectionName: "test",
